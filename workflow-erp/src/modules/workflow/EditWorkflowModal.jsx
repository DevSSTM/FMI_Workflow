...
const EditWorkflowModal = ({ isOpen, onClose, workflow }) => {
  ...
  const { updateWorkflow, isLoading } = useWorkflowStore();
  ...
  // Initialize form with workflow data when modal opens
  useEffect(() => {
    if (isOpen && workflow) {
      fetchUsers();
      setValue('name', workflow.name || '');
      setValue('description', workflow.description || '');
      setSelectedDepartments(workflow.departments || [workflow.department]);
      ...
      if (workflow.steps && workflow.steps.length > 0) {
        setSteps(workflow.steps.map(step => ({
          ...step,
          documents: [],
          portalEnabled: step.portalEnabled || false,
          directUploadEnabled: step.directUploadEnabled || false,
          portalDeadline: step.portalDeadline || '',
        })));
      } else {
        setSteps([createStep(`step-${Date.now()}`, 1)]);
      }
    }
  }, [isOpen, workflow, fetchUsers, setValue]);
  ...
  const onSubmit = async (data) => {
    ...
    const result = await updateWorkflow(workflow.id, workflowData);
    if (result.success) {
      // Upload new documents for each step
      ...
      showError(result.error || 'Failed to update workflow.');
    }
  };
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Edit Workflow" size="xl">
