...
set password_hash = case username
  when 'admin' then extensions.crypt('admin', extensions.gen_salt('bf'))
  when 'panchali' then extensions.crypt('panchali123', extensions.gen_salt('bf'))
  when 'nirmal' then extensions.crypt('nirmal123', extensions.gen_salt('bf'))
  when 'sewmi.hiruni' then extensions.crypt('sewmi123', extensions.gen_salt('bf'))
  when 'isma' then extensions.crypt('isma123', extensions.gen_salt('bf'))
  else password_hash
end
where username in ('admin', 'panchali', 'nirmal', 'sewmi.hiruni', 'isma');
